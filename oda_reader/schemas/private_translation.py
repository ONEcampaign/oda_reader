...
MAPPINGS = {
    "private_codes_area": ImporterPaths.mappings / "private_codes_area.json",
    "area_code_corrections": ImporterPaths.mappings / "area_code_corrections.json",
}


def update_private_translation_mappings():
    ...
    extract_dac_to_area_codes(xml_dict=xml_data, filename=MAPPINGS["private_codes_area"])


def area_code_mapping() -> dict:
    """Reads the area code mapping."""
    return read_mapping(
        MAPPINGS["private_codes_area"],
        keys_as_int=True,
        update=update_private_translation_mappings,
    ) | read_mapping(
...
def convert_private_to_dotstat_codes(df: pd.DataFrame) -> pd.DataFrame:
    """Convert the Private donor data to the .stat schema.
    Args:
        df (pd.DataFrame): The Private donor data.

    Returns:
        pd.DataFrame: The Private donor data in the .stat schema.
    """
    # # Get the area codes
